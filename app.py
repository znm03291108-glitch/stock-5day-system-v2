...
def handle_exception(e):
    return jsonify({
        ...
        "version": "3.7.3.6-watchpool-before-empty",
...
# ===== V3.7.3.1 stronger clean helpers =====
def clean_badge_v3731(item: Dict[str, Any]) -> str:
    x = item or {}
    if x.get("valid_for_5day_system") is False or x.get("valid_ma5") is False:
        return "已过滤"
    if x.get("safe_buy_point"):
        return "安全买点"
    if x.get("near_buy_point"):
        return "接近买点"
    if x.get("main_result_visible"):
        return "有效5日线"
    return "候选"

def force_strong_clean_v3731(item: Dict[str, Any]) -> Dict[str, Any]:
    x = normalize_clean_card_v373(item)
    reasons = list(x.get("invalid_reasons") or [])

    name = safe_str(x.get("name")).upper()
    symbol = safe_str(x.get("symbol") or x.get("code"))
    ...
    # 再兜底一次：只要 MA5 无效，就不允许进入主股票卡
    if ma5 is None or close is None or ma5 <= 0 or close <= 0:
        reasons.append("MA5缺失或无效")
    if name.startswith(("N", "C", "U", "W")) or "新股" in name or "次新" in name:
        reasons.append("新股/特殊上市标识")
    ...
    x["main_result_visible"] = valid
    x["quick_candidate_only"] = not valid
    x["badge"] = clean_badge_v3731(x)
    x["tag"] = x["badge"]

    if not valid:
        x["score"] = min(int(to_float_safe(x.get("score") or x.get("smart_score"), 49) or 49), 49)
        x["smart_score"] = x["score"]
        x["operation_advice"] = "快速候选，仅说明热度，不代表买点；已触发过滤：" + "、".join(reasons)
        x["advice"] = x["operation_advice"]
        x["position_advice"] = "不买；不加仓；如已有底仓，仅作观察。"
        x["position"] = x["position_advice"]
        x["risk_advice"] = "、".join(reasons) + "；不参与5日线交易计划。"
        x["risk_text"] = x["risk_advice"]
        x["trading_plan"] = "快速候选尚未计算有效5日线，不能生成实盘交易计划。请先深度分析。"
        x["trading_plan_text"] = x["trading_plan"]
    else:
        x["badge"] = clean_badge_v3731(x)
        x["tag"] = x["badge"]
...
def clean_results_v3731(items, default_only_valid: bool = True, max_items: int = None):
    cleaned = [force_strong_clean_v3731(x) for x in (items or [])]
    valid = [x for x in cleaned if x.get("valid_for_5day_system") is True and x.get("valid_ma5") is True]
    removed = [x for x in cleaned if x not in valid]
    ...
    return {
        ...
        "clean_stats": {
            ...
            "removed_count": len(removed),
            "total_count": len(cleaned),
            "mode": "强过滤：主列表只显示有效5日线"
        }
    }
# ===== end V3.7.3.1 helpers =====



# ===== V3.7.3.4 empty result + filter reason summary =====
def filter_reason_summary_v3734(items):
    summary = {
        "MA5缺失或无效": 0,
        "新股/特殊上市标识": 0,
        "ST/退市风险": 0,
    ...
    valid = []
    for raw in (items or []):
        x = force_strong_clean_v3731(raw)
        if x.get("valid_for_5day_system") and x.get("valid_ma5"):
            valid.append(x)
        else:
            removed.append(x)
            reasons = x.get("invalid_reasons") or []
            if not reasons:
                summary["其他"] += 1
            for r in reasons:
                hit = False
                for k in list(summary.keys()):
                    if k != "其他" and k in str(r):
                        summary[k] += 1
                        hit = True
                        break
                if not hit:
    ...
    return {
        "valid_count": len(valid),
        "removed_count": len(removed),
        "total_count": len(valid) + len(removed),
        "reason_summary": summary,
        "empty_message": "今日暂无有效5日线股票，系统已过滤 MA5缺失/新股/ST/异常涨幅股票。可点击查看已过滤复核，或下一个交易日重新筛选。"
    }

def clean_results_v3734(items, default_only_valid: bool = True, max_items: int = None):
    base = clean_results_v3731(items, default_only_valid=default_only_valid, max_items=max_items)
    base["filter_summary"] = filter_reason_summary_v3734(items)
    base["empty_message"] = base["filter_summary"]["empty_message"]
    return base
# ===== end V3.7.3.4 =====
...
def api_health():
    return jsonify({"ok": True, "service": "stock-5day-system-v2", "version": "3.7.3.6-watchpool-before-empty", "time": datetime.now().isoformat(timespec="seconds"), "message": "后端正常，支持财报结果中文解释、交易日状态识别、大盘情绪联动与实盘交易计划"})
...
def api_batch_analyze():
    ...
    return jsonify({"ok": True, "version": "3.7.3.6-watchpool-before-empty", "summary": build_summary(results, len(symbols), len(errors)), "results": clean_results_v3734(results)["results"], "clean_stats": clean_results_v3734(results)["clean_stats"], "removed_results": clean_results_v3734(results)["removed_results"], "errors": errors})
...
def api_trading_status():
    try:
        ...
        return jsonify({"ok": False, "version": "3.7.3.6-watchpool-before-empty", "error": str(e), "type": e.__class__.__name__}), 200
...
def api_theme_stocks():
    ...
    try:
        ...
        return jsonify({
            ...
            "results": clean_results_v3734(results)["results"], "clean_stats": clean_results_v3734(results)["clean_stats"], "removed_results": clean_results_v3734(results)["removed_results"],
        })
    except Exception as e:
        return jsonify({
            "ok": False,
...
def api_smart_hot():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": str(e), "type": e.__class__.__name__, "where": "eastmoney_spot", "hint": "东方财富实时行情接口暂时不可用。稍后重试，或先用单股分析。", "version": "3.7.3.6-watchpool-before-empty", "summary": build_summary([], 0, 1), "themes": [], "results": [], "errors": [{"error": str(e)}]}), 200
    ...
    return jsonify({"ok": True, "version": "3.7.3.6-watchpool-before-empty", "mode": "risk_filter_quick_first", "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "summary": summary, "themes": try_fetch_theme_board(limit=20), "results": quick_results[:quick_limit], "errors": [{"info": x} for x in spot_data.get("errors", [])]})
...
def api_deep_batch():
    ...
    results.sort(key=lambda x: (x.get("rank", 9), -int(x.get("smart_score", 0)), -(x.get("quote", {}).get("pct_chg") or 0)))
    return jsonify({"ok": True, "version": "3.7.3.6-watchpool-before-empty", "offset": offset, "size": size, "next_offset": next_offset, "done": done, "total": len(symbols), "summary": build_summary(results, len(batch), len(errors)), "results": clean_results_v3734(results)["results"], "clean_stats": clean_results_v3734(results)["clean_stats"], "removed_results": clean_results_v3734(results)["removed_results"], "errors": errors})
...
def api_clean_filter_status():
    ...
    return jsonify({
        ...
        "demo": clean_results_v3734(demo, default_only_valid=False)
    })


@app.route("/api/strong_clean_status", methods=["GET"])
def api_strong_clean_status():
    demo = [
        {"symbol":"001399", "name":"N惠科", "pct_chg":315.02, "close":42.00, "ma5":None, "score":95, "turnover":65.92},
        {"symbol":"300720", "name":"海川智能", "pct_chg":10.41, "close":88.88, "ma5":None, "score":95, "amount":15.1},
        {"symbol":"300592", "name":"华凯易佰", "pct_chg":12.94, "close":16.93, "ma5":15.31, "score":80, "amount":4.9}
    ]
    return jsonify({
        "ok": True,
...
@app.route("/api/final_card_filter_status", methods=["GET"])
def api_final_card_filter_status():
    demo = [
        {"symbol":"001399", "name":"N惠科", "pct_chg":315.02, "close":42.00, "ma5":None, "score":95, "turnover":65.92},
        {"symbol":"300264", "name":"佳创视讯", "pct_chg":20.02, "close":9.77, "ma5":None, "score":88, "amount":4.7},
        {"symbol":"300592", "name":"华凯易佰", "pct_chg":12.94, "close":16.93, "ma5":15.31, "score":80, "amount":4.9}
    ]
...
@app.route("/api/dom_final_clean_status", methods=["GET"])
def api_dom_final_clean_status():
    demo = [
        {"symbol":"001399", "name":"N惠科", "pct_chg":315.02, "close":42.00, "ma5":None, "score":95, "turnover":65.92},
        {"symbol":"300162", "name":"雷曼光电", "pct_chg":19.98, "close":12.13, "ma5":None, "score":87, "amount":12.2},
        {"symbol":"300592", "name":"华凯易佰", "pct_chg":12.94, "close":16.93, "ma5":15.31, "score":80, "amount":4.9}
    ]
    return jsonify({
        "ok": True,
...
@app.route("/api/empty_filter_summary_status", methods=["GET"])
def api_empty_filter_summary_status():
    demo = [
        {"symbol":"001399", "name":"N惠科", "pct_chg":315.02, "close":42.00, "ma5":None, "score":95, "turnover":65.92},
        {"symbol":"300162", "name":"雷曼光电", "pct_chg":19.98, "close":12.13, "ma5":None, "score":87, "amount":12.2},
        {"symbol":"300338", "name":"*ST开元", "pct_chg":17.39, "close":2.97, "ma5":None, "score":23, "amount":0.7602},
        {"symbol":"300592", "name":"华凯易佰", "pct_chg":12.94, "close":16.93, "ma5":15.31, "score":80, "amount":4.9}
    ]
    return jsonify({
        "ok": True,
...
@app.route("/api/watchpool_empty_status", methods=["GET"])
def api_watchpool_empty_status():
    return jsonify({
        "ok": True,
        "version": "3.7.3.6-watchpool-before-empty",
        "fixes": [
        ...
        "message": "如果主股票卡没有有效5日线结果，会在观察池上方强制显示空结果提示。"
    })
